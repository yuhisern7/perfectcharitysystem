...
# Placeholders so ML-related names are always defined for type checkers
np: Any = None
IsolationForest: Any
...
# Persistent storage paths (always under the shared data directory)
_DATA_DIR = pcs_persistence.DATA_DIR
_THREAT_LOG_FILE = _DATA_DIR / "threat_log.json"
...
_username_tracker: Dict[str, set[str]] = defaultdict(set)

# ============================================================================
# REAL AI/ML MODELS - Machine Learning Security Intelligence
# ============================================================================

# ML Model storage paths (under data/ml_models relative to project root)
_ML_MODELS_DIR = _DATA_DIR / "ml_models"
_ANOMALY_MODEL_FILE = _ML_MODELS_DIR / "anomaly_detector.pkl"
_THREAT_CLASSIFIER_FILE = _ML_MODELS_DIR / "threat_classifier.pkl"
_IP_REPUTATION_FILE = _ML_MODELS_DIR / "ip_reputation.pkl"
_SCALER_FILE = _ML_MODELS_DIR / "feature_scaler.pkl"

# ML Models (initialized lazily)
_anomaly_detector: Any = None  # IsolationForest for zero-day attack detection
_threat_classifier: Any = None  # RandomForest for multi-class threat classification
_ip_reputation_model: Any = None  # GradientBoosting for IP reputation scoring
_feature_scaler: Any = None  # StandardScaler for feature normalization
_ml_training_data: list[Any] = []  # Training data buffer
_ml_last_trained: Optional[datetime] = None  # Last training timestamp
_ml_prediction_cache: Dict[str, Any] = {}  # Cache for ML predictions

# ML Feature extraction tracking
_request_features: Dict[str, List[Any]] = defaultdict(list) if ML_AVAILABLE else defaultdict(list)
...
def _save_threat_log() -> None:
    ...
    try:
        pcs_persistence.ensure_data_directory()
        with _THREAT_LOG_FILE.open('w', encoding='utf-8') as f:
...
def _save_blocked_ips() -> None:
    ...
    try:
        pcs_persistence.ensure_data_directory()
        with _BLOCKED_IPS_FILE.open('w', encoding='utf-8') as f:
...
def _load_threat_data() -> None:
    ...
    # Load threat log
    try:
        if _THREAT_LOG_FILE.exists():
            with _THREAT_LOG_FILE.open('r', encoding='utf-8') as f:
    ...
    try:
        if _BLOCKED_IPS_FILE.exists():
            with _BLOCKED_IPS_FILE.open('r', encoding='utf-8') as f:
...
def _extract_features_from_request(ip_address: str, endpoint: str, user_agent: str, 
                                   headers: Dict[str, Any], method: str = "GET") -> Any:
...
def _ml_predict_anomaly(features: Any) -> Tuple[bool, float]:
...
def _ml_classify_threat(features: Any) -> Tuple[str, float]:
...
def _ml_predict_ip_reputation(features: Any) -> Tuple[bool, float]:
...
def get_ml_model_stats() -> dict:
    ...
    if _anomaly_detector is not None:
        model_stats = {
            ...
            "n_estimators": getattr(_anomaly_detector, 'n_estimators', 0),
    ...
    if _threat_classifier is not None:
        model_stats = {
            ...
            "n_estimators": getattr(_threat_classifier, 'n_estimators', 0),
    ...
    if _ip_reputation_model is not None:
        model_stats = {
            ...
            "n_estimators": getattr(_ip_reputation_model, 'n_estimators', 0),
...
def retrain_ml_models_now() -> dict:
    ...
    try:
        ...
        return {
            "success": True,
            "trained_at": _ml_last_trained.isoformat() if _ml_last_trained else None,
...
def _ensemble_predict(features: Any) -> Tuple[bool, float, dict]:
...
def _detect_vpn_tor_proxy(ip_address: str, headers: Dict[str, Any]) -> dict:
...
def _fingerprint_client(ip_address: str, user_agent: str, headers: Dict[str, Any], behavioral_data: Optional[Dict[str, Any]] = None) -> str:
...
def _log_threat(ip_address: str, threat_type: str, details: str, level: ThreatLevel, action: str = "monitored", headers: Optional[Dict[str, Any]] = None) -> None:
...
def is_credential_stuffing(username: str, ip_address: str) -> bool:
    ...
    global _username_tracker
    _username_tracker[ip_address].add(username)
	
    # If same IP tries more than 5 different usernames in short time, it's stuffing
    if len(_username_tracker[ip_address]) > 5:
        _log_threat(
            ip_address=ip_address,
            threat_type="Credential Stuffing",
            details=f"Attempted {len(_username_tracker[ip_address])} different usernames",
            level=ThreatLevel.CRITICAL,
            action="blocked"
        )
        _block_ip(ip_address)
...
def generate_tracking_headers(ip_address: str, session_id: Optional[str] = None) -> dict:
    ...
    if session_id is None:
        local_session_id = secrets.token_hex(16)
    else:
        local_session_id = session_id
	
    tracking_token = _create_tracking_beacon(ip_address, local_session_id)
